...
def getWeighting (ticker, inter):
    ...
    def leastSquares(xVals, yVals, indicator, plots):
        ...
        cov = np.sum((xVals - np.mean(xVals))*(yVals - np.mean(yVals)))/np.size(yVals) #this doesn't seem to be right...
        ...
        if plots == True:
            ...
            plt.figtext(1, 0.03, "Covariance: " + str(np.round(cov, 2)), horizontalalignment = 'right', fontweight = 'extra bold')
    ...
    # Linear Combination
    w = np.array([1/MACD.cov, 1/RSI.cov, 1/CCI.cov, 1/stoch.cov, 1/ADX.cov, 1/aroon.cov, 1/AD.cov, 1/OBV.cov])
    w = w/w.sum()
